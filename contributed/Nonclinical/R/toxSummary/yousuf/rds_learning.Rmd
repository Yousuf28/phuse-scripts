...
find_rev <- c("[PR]", "[REV]")
find_rev_clean <- gsub('\\[|\\]', '',find_rev)
find_rev_clean
 
```


```{r create plotData, eval=FALSE, include=FALSE}

plotData <- data.frame(matrix(ncol = 13 ))
column_names <- c("Study", "Species", "Months", "Dose", "NOAEL", "Cmax", "AUC", "Findings", "Reversibility", "Severity", "Value", "NOAEL_all", "Key_findings")
  ...
  if (Study != "New Study") {
    ...
    for (i in seq(studyData$nFindings)){
      for (j in seq(studyData$nDoses)){
        ...
        plotData[count, "Key_findings"] <- studyData[["Findings"]][[paste0("Finding", i)]][["Finding"]]
