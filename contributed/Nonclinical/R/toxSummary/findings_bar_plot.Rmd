...
```{r safety margin and findings plot__ plotly, eval=FALSE, include=FALSE}
...
marker_man <- list(size = 10,
                             color = 'rgba(255, 182, 193, .9)',
                             line = list(color = 'rgba(152, 0, 0, .8)',
                                      width = 2))
  dose <- as.character(plotData$Dose)

 pal <- c("black", "#239B56")  
  
fig_plotdata <- plotData %>% 
  group_by(Study) %>% 
  group_map(~ plot_ly(data = ., x = ~ SM, y= ~Value_order, color = ~NOAEL, text= ~paste(Dose,"mg/kg/day"), type = "scatter", mode= "markers+text", symbols = "square-dot",colors = pal,  marker = list(size = 50, symbol = "square-open"))) %>%
...
color_NOAEL <- c("TRUE" = "#239B56", "FALSE" = "black")
  ...
  geom_tile(aes (x = SM, y = Value_order, fill = NOAEL), 
            color = "transparent", width = 0.35, height = 0.6)+
    facet_trelliscope(~ Study, as_plotly = TRUE)
```


```{r}
mtcars %>% 
  ggplot(aes(x = mpg, y= hp, fill = gear))+
  geom_point()
```
