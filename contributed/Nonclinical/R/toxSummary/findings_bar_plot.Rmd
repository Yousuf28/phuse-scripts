...
### ggplotly function does not work with geom_label

```{r plotly with geom_label in safety margin plot  ,  fig.width=16, fig.height=12}


p <- ggplot(plotData )+ 
  geom_rect(mapping = aes(xmin = SM-20, xmax = SM+20,ymin= SM -.5, ymax= SM+.5 ))+
    geom_text(aes(x= SM, y= Value, x = SM, y = Value, label = paste(Dose, " mg/kg/day")),
...
```{r geom_tile}
p <- ggplot(plotData )+ 
  geom_tile(mapping = aes(x = SM, y= Value ))+
